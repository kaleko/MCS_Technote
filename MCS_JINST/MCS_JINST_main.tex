...
% temporary red text to show changes
\usepackage{color}
...
\caption{\textcolor{red}{INTERAL NOTE TO COLLABORATORS [the following text will not be included in the submitted paper draft]: This figure has changed. Raw std and mean are drawn on plots now. Note the raw std and mean are much worse than the results from the Gaussian fit. This is because we cut out tails which skew the std and mean significantly when computing the bias and resolution via Gaussian fits.} Fractional momentum difference for a few representative bins of true momentum for a sample of simulated exiting muon tracks. The y-axis is number of tracks, and the x-axis is $(p_{\text{MCS}}^{-1} - p_{\text{true}}^{-1})/(p_{\text{true}}^{-1})$.}
